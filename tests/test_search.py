...
from ld_audit.config import DEFAULT_MAX_FILE_SIZE_MB
...
class TestCodebaseScanner:
    def test_search_file_single_flag(self):
        ...
        try:
            result = scanner._search_file_with_encoding(temp_path, ["test-flag-1"], "utf-8")
            assert "test-flag-1" in result
            assert len(result["test-flag-1"]) == 2
            assert result["test-flag-1"][0].line_number == 1
            assert result["test-flag-1"][1].line_number == 2
        finally:
            os.unlink(temp_path)
    ...
    def test_search_file_multiple_flags(self):
        ...
        try:
            result = scanner._search_file_with_encoding(temp_path, ["feature-a", "feature-b"], "utf-8")
    ...
    def test_search_file_no_matches(self):
        ...
        try:
            result = scanner._search_file_with_encoding(temp_path, ["nonexistent-flag"], "utf-8")
            assert result == {}
        finally:
            os.unlink(temp_path)
    ...
    def test_search_file_single_quotes(self):
        ...
        try:
            result = scanner._search_file_with_encoding(temp_path, ["single-quote-flag"], "utf-8")
...
class TestSearchDirectory:
    def test_search_directory_basic(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = scanner.search_directory(tmpdir, ["my-flag"])
    ...
    def test_search_directory_with_extension_filter(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = scanner.search_directory(tmpdir, ["filter-flag"], extensions=["py"])
    ...
    def test_search_directory_recursive(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = scanner.search_directory(tmpdir, ["recursive-flag"])
    ...
    def test_search_directory_excludes_common_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = scanner.search_directory(tmpdir, ["excluded-flag"])
    ...
    def test_search_directory_multiple_flags(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = scanner.search_directory(tmpdir, ["flag-one", "flag-two", "flag-three"])
    ...
    def test_search_directory_no_matches(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = scanner.search_directory(tmpdir, ["nonexistent-flag"])
    ...
    def test_search_directory_skips_large_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(large_file, "w") as f:
                f.write("x" * (DEFAULT_MAX_FILE_SIZE_MB * 1024 * 1024 + 1000))

            result = scanner.search_directory(tmpdir, ["any-flag"])
