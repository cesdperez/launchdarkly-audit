...
class TestGetEnvValue:
    def test_get_env_value_with_default_env(self):
        flag = {
            'environments': {
                'production': {'on': True, 'lastModified': 1234567890000}
            }
        }
        assert get_env_value(flag) is True
        assert get_env_value(flag, key='lastModified') == 1234567890000
    ...
    def test_get_env_value_missing_key(self):
        ...
        assert get_env_value(flag, key='missing', default='fallback') == 'fallback'

    def test_get_env_value_no_environments(self):
        flag = {}
        assert get_env_value(flag, default=False) is False
...
class TestGetPrimaryEnv:
    def test_get_primary_env_with_production(self):
        ...
        assert get_primary_env(flag) == 'production'
    ...
    def test_get_primary_env_fallback_to_staging(self):
        ...
        assert get_primary_env(flag) == 'staging'
    ...
    def test_get_primary_env_fallback_to_dev(self):
        ...
        assert get_primary_env(flag) == 'dev'
        ...
        flags = [
            {
                ...
                'environments': {
                    'production': {
                        'on': False,
                        'lastModified': int(six_months_ago.timestamp() * 1000)
                    }
                }
            }
        ]
        ...
        flags = [
            {
                ...
                'environments': {
                    'production': {
                        'on': False,
                        'lastModified': int(one_month_ago.timestamp() * 1000)
                    }
                }
            }
        ]
